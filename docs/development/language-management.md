...
- 한국어 노드 정의 번역은 `locales/ko/nodeDefs.json`에 둔다.
...
3. `locales/ko/nodeDefs.json`에 같은 class id의 한국어 항목을 추가한다.
4. locale key는 `NODE_CLASS_MAPPINGS`의 class id와 정확히 일치해야 한다.
5. required input과 output index가 빠지지 않았는지 확인한다.
6. `nodeDefs.json`을 JSON으로 검증한다.
