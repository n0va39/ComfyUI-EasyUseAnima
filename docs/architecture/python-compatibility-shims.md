...
| `autocomplete_dataset.py` | Partial explicit compatibility module after D-11b; prompt classification remains root-owned with its D-13 dependency removed | `easyuse_anima.autocomplete.dataset` and `.search`; final classification owner is now unblocked | #162, #186 D-11 | Existing 0.5.2 dataset/search surface canonicalized in PR #386 with direct identity aliases; D-13 switched classification to canonical ANIMA prompt imports | External/legacy imports and prompt tests; `api.py` uses canonical dataset/search and root classification | Unscheduled; final classification identity surface, N+1 gate, and classification/result/API parity |
