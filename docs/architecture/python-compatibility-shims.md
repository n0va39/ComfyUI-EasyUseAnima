...
| `api.py` route-registration surface | Partial route adapter after D-03a/D-03b plus explicit D-10 profile aliases; planned API shim | `easyuse_anima.profiles.*`, `easyuse_anima.api.routes.translation`, then remaining routes/router | #163, #165, #186 D-10/D-02-D-07 | Existing 0.5.2 route surface; D-03a canonicalizes the translation handler factory and D-03b its bounded executor while root retains singleton lifecycle/correlation/registration composition | Root entrypoint side-effect import, frontend endpoints, profile/API tests | Unscheduled; remaining route ownership, N+1 gate, and route parity |
