...
| `api.py` route-registration surface | Partial route adapter after D-03a/D-03b/D-04a plus explicit D-10 profile aliases; planned API shim | `easyuse_anima.profiles.*`, `easyuse_anima.api.routes.translation`, `.aio_torch_compile`, then remaining routes/router | #163, #165, #186 D-10/D-02-D-07 | Existing 0.5.2 route surface; D-03 canonicalizes translation handler/executor and D-04a the read-only AiO Torch Compile handler while root retains lifecycle/correlation/registration composition | Root entrypoint side-effect import, frontend endpoints, profile/API tests | Unscheduled; remaining route ownership, N+1 gate, and route parity |
