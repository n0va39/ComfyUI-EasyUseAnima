...
| `wildcard_engine.py` | D-12 canonical facade plus root-owned snapshot cache/single-flight lifecycle; not yet an ADR-002 direct re-export shim | `easyuse_anima.wildcard.models`, `.sources`, `.snapshot`, `.seed`, `.mode`, `.selector`, `.library`, and `.expansion`; E-06 owns remaining lifecycle | #184, #186 D-12, #187 E-06 | D-12 completed in PR #469; v0.6.1/v0.6.2 contain the canonical expansion path and root facade, not the final lifecycle split | `nodes.py`, `api.py`, canonical runtime resolvers, wildcard/workflow tests | Blocked and retained; E-06 ownership, internal-consumer removal, a final identity surface, and a later release window are required |
...
- the root entrypoint is permanent and every compatibility surface remains retained;
- `nodes.py`, `settings.py`, `storage.py`, `autocomplete_index.py`, and
  `prompt_translation.py` have a complete canonical-plus-shim release window, but
...
- `api.py` remains the imported API composition/runtime facade, and
  `wildcard_engine.py` still owns the E-06 snapshot lifecycle and has direct
  production consumers; neither is a removable direct re-export shim; and
