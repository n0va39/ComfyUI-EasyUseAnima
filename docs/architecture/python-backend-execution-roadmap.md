...
- Snapshot branch: `dev`
- Integrated `dev` snapshot commit: `8183aaf472d5cf88978c1dfe20f7bc5ca7d1895c`
...
| D - root consolidation | D-01, D-08, D-09, and D-10 complete on `dev`; D-11a/#385, D-11b/#386, and D-12a/#387 through D-12e/#391 validated; D-12f1/#392 selector import contract VALIDATED; D-12f2 selector Move READY | Complete D-12f2 before lifecycle/expansion and D-13; finish D-11 classification only after D-13 removes its root-package dependency |
...
| 18 | D-series canonical root consolidation | D-01 translation, D-08 filesystem, D-09 settings, and D-10 profiles COMPLETE on `dev`; D-11a/#385 and D-11b/#386 autocomplete VALIDATED; D-12a/#387 models through D-12e/#391 mode contract VALIDATED; D-12f1/#392 selector import contract VALIDATED; D-12f2 selector Move READY | Contract then Move | #186 | Phase B exit; per-feature behavior stable |
...
PR #358 implements that service boundary, composes one process-lifetime owner
without adding production callers, and passes its one official full gate.
It is complete on `dev` at `d61749a`. The S167-03 pre-adapter audit found two
missing decisions: headless execution request identity and settlement timing.
S167-03a now freezes those decisions plus cache, compatibility, display, split,
...
S167-03d connects Advanced, Advanced v2, and Regional Prompt Studio to that
session. The backend now owns execution/next-seed selection and accepted
publication, fixed workflows replay their concrete saved seed independently of
retained advancing state, and non-fixed controls force execution. The retired
browser queue reservation runtime and bridge are removed; old hidden payloads
are still scrubbed but cannot choose the next seed. PR #362 completes that
rollback unit on `dev`.

S167-03e connects `EasyUseAnimaAIOGenerator` to the same backend
identity/session/service boundary using the legacy version-2 parser, AiO's
existing `2^50` clamp domain, and explicit cache forcing for special or
non-fixed controls. The concrete reservation seed enters legacy orchestration
before any sampling or cache key is computed. A decimal-string executed
payload supplies last-executed and accepted next-seed display without changing
the three outputs or metadata schema. The browser `WeakMap`/FIFO queue service,
workflow rewrite, and global queue hook are retired; a small DOM-free executed
display adapter remains.
...
reservation consumers, and expansion remain root-owned or unchanged. D-12e
moves the standalone and Prompt Studio mode constants, labels, legacy alias
lookup, and normalization functions to `easyuse_anima.wildcard.mode`. Root
retains direct aliases, and selector/NumPy import timing, lifecycle, expansion,
adapter literals, and consumer imports remain unchanged. D-12f1 freezes the
next selector Move's split import contract: root keeps its eager `numpy as np`
