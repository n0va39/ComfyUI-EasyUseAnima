...
class AIOFrontendSourceTests(unittest.TestCase):
    def test_generator_preview_uses_bounded_viewport_height(self):
        ...
        self.assertIn("|| isNodeUserResizeActive(node);", source)
        self.assertIn("node.size[0] = targetWidth;", source)
        self.assertNotIn("currentHeight >= minHeight - 1", source)
        self.assertNotIn("Math.abs(currentHeight - minHeight) > 1", source)
        self.assertNotIn("Math.max(currentHeight, minHeight)", source)
    ...
    def test_generator_layout_maps_user_height_to_panel_viewport(self):
        ...
        self.assertIn("|| isNodeUserResizeActive(node);", body)
        self.assertNotIn("generatorMinimumNodeHeight(node)", body)
        self.assertNotIn("const currentHeight =", body)
        self.assertNotIn("currentHeight >= minHeight - 1", body)
        ...
        self.assertNotIn("Math.max(currentHeight, minHeight)", body)
    ...
    def test_generator_available_height_uses_node_size(self):
        ...
        self.assertIn("if (isNodeUserResizeActive(node))", panel_height_body)
        self.assertIn("return generatorNodeAvailablePanelHeight(node);", panel_height_body)
    ...
    def test_generator_manual_resize_and_min_width_are_explicit(self):
        ...
        self.assertIn("node.size[0] = targetWidth;", min_width_body)
        self.assertIn("markNodeDirty(node);", min_width_body)
        ...
        self.assertIn("./prompt_studio/node_resize_tracking.js", source)
        self.assertIn("installNodeResizePointerTracker,", source)
        self.assertIn("isNodeUserResizeActive,", source)
        self.assertIn("installNodeResizePointerTracker();", source)
        self.assertIn("if (isNodeUserResizeActive(this))", on_resize_body)
        ...
        self.assertIn('{ capture: true, passive: false });', source)
