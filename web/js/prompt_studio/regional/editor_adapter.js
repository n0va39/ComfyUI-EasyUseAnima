...
/**
 * @typedef {import("../types.js").PromptStudioAdvancedTextarea & {
 *   __easyuseAnimaPromptStudioResizeStartHeight?: number,
 * }} RegionalPromptStudioTextarea
 */
...
const PROMPT_STUDIO_COMMON_TEXT = {
  en: {
    ...
    "advanced.wildcardTitle": "Choose the wildcard selection mode, seed, and next-run seed rule.",
    "advanced.wildcardSeed": "Wildcard seed",
    "advanced.wildcardSeedControl": "Seed control",
    "advanced.wildcardMode.populateTitle": "General: the same source and seed produce the same result.",
    "advanced.wildcardMode.sequentialTitle": "Sequential: the seed selects each option by position; a fixed seed reproduces the same result.",
    "advanced.wildcardSeedTitle": "The same source, mode, and seed produce the same result.",
    "advanced.wildcardSeedControl.fixed": "Fixed",
    "advanced.wildcardSeedControl.randomize": "Randomize every time",
    "advanced.wildcardSeedControl.increment": "Increment",
    "advanced.wildcardSeedControl.fixedTitle": "Keep the current seed after an accepted queue.",
    "advanced.wildcardSeedControl.randomizeTitle": "Choose a random seed for the next run after an accepted queue.",
    "advanced.wildcardSeedControl.incrementTitle": "Add 1 to the seed after an accepted queue and wrap to 0 after the maximum.",
  ...
  ko: {
    ...
    "advanced.wildcardTitle": "와일드카드 선택 방식, 시드, 다음 실행 시드 규칙을 설정합니다.",
    "advanced.wildcardSeed": "와일드카드 시드",
    "advanced.wildcardSeedControl": "시드 제어",
    "advanced.wildcardMode.populateTitle": "일반: 같은 원본과 같은 시드는 같은 결과를 만듭니다.",
    "advanced.wildcardMode.sequentialTitle": "순차: 시드값을 항목의 순서 위치로 사용하며, 고정 시드는 같은 결과를 재현합니다.",
    "advanced.wildcardSeedTitle": "같은 원본, 같은 모드, 같은 시드는 같은 결과를 만듭니다.",
    "advanced.wildcardSeedControl.fixed": "고정",
    "advanced.wildcardSeedControl.randomize": "매번 랜덤",
    "advanced.wildcardSeedControl.increment": "증가",
    "advanced.wildcardSeedControl.fixedTitle": "큐가 정상 수락된 뒤에도 현재 시드를 유지합니다.",
    "advanced.wildcardSeedControl.randomizeTitle": "큐가 정상 수락되면 다음 실행 시드를 무작위로 바꿉니다.",
    "advanced.wildcardSeedControl.incrementTitle": "큐가 정상 수락되면 시드를 1 올리고 최댓값 다음에는 0으로 돌아갑니다.",
  ...
  ja: {
    ...
    "advanced.wildcardTitle": "ワイルドカードの選択方式、シード、次回実行のシード規則を設定します。",
    "advanced.wildcardSeed": "ワイルドカードシード",
    "advanced.wildcardSeedControl": "シード制御",
    "advanced.wildcardMode.populateTitle": "通常: 同じ元テキストとシードは同じ結果になります。",
    "advanced.wildcardMode.sequentialTitle": "順次: シードを項目の順番として使い、固定シードは同じ結果を再現します。",
    "advanced.wildcardSeedTitle": "同じ元テキスト、モード、シードは同じ結果になります。",
    "advanced.wildcardSeedControl.fixed": "固定",
    "advanced.wildcardSeedControl.randomize": "毎回ランダム",
    "advanced.wildcardSeedControl.increment": "増加",
    "advanced.wildcardSeedControl.fixedTitle": "キュー受理後も現在のシードを維持します。",
    "advanced.wildcardSeedControl.randomizeTitle": "キュー受理後、次回実行のシードをランダムに変更します。",
    "advanced.wildcardSeedControl.incrementTitle": "キュー受理後にシードを 1 増やし、最大値の次は 0 に戻ります。",
  ...
  zh: {
    ...
    "advanced.wildcardTitle": "设置通配符选择模式、种子和下次运行的种子规则。",
    "advanced.wildcardSeed": "通配符种子",
    "advanced.wildcardSeedControl": "种子控制",
    "advanced.wildcardMode.populateTitle": "普通：相同源文本和种子会产生相同结果。",
    "advanced.wildcardMode.sequentialTitle": "顺序：使用种子作为选项位置，固定种子可复现相同结果。",
    "advanced.wildcardSeedTitle": "相同源文本、模式和种子会产生相同结果。",
    "advanced.wildcardSeedControl.fixed": "固定",
    "advanced.wildcardSeedControl.randomize": "每次随机",
    "advanced.wildcardSeedControl.increment": "增加",
    "advanced.wildcardSeedControl.fixedTitle": "队列接受后保持当前种子。",
    "advanced.wildcardSeedControl.randomizeTitle": "队列接受后为下次运行随机选择种子。",
    "advanced.wildcardSeedControl.incrementTitle": "队列接受后将种子加 1，超过最大值后回到 0。",
...
function ensurePromptStudioHighlightStyle() {
...
function refreshPromptStudioCommonHighlightInputs() {
  for (const input of document.querySelectorAll(".easyuse-anima-highlight-input")) {
    const highlightInput = /** @type {PromptStudioInputElement} */ (input);
    highlightInput.__easyuseAnimaHighlightRefresh?.(true);
  }
}
...
async function loadPromptStudioCommonSettings() {
  try {
    ...
    if (settings) {
      applyPromptStudioCommonSettings(settings);
...
let promptStudioRegionalAdapterInstalled = false;

export function installPromptStudioRegionalAdapter() {
  if (promptStudioRegionalAdapterInstalled || typeof window === "undefined") {
    return;
  }
  promptStudioRegionalAdapterInstalled = true;
  loadPromptStudioCommonSettings();
  promptStudioWindow().addEventListener("easyuse-anima-settings-updated", (event) => {
    const settings = event instanceof CustomEvent ? event.detail : {};
    applyPromptStudioCommonSettings(settings);
    refreshPromptStudioCommonHighlightInputs();
  });
}
...
const renderHighlightedText = createPromptHighlightRenderer({
  ...
  tokenStyle,
  tokenSpanHtml,
  weightSyntaxUnderlineEnabled: () => PROMPT_STUDIO_COMMON_SETTINGS.weightSyntaxUnderline,
 ...
 * @param {any[] | null} [tokens]
 * @param {boolean} [forceCopyMetrics]
