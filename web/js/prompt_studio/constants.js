...
const PROMPT_STUDIO_TEXT = {
  en: {
    ...
    "advanced.wildcardMode.populateTitle": "General: expand the original source fields deterministically. The same source and seed produce the same result, the seed stays fixed, and the workflow keeps the source fields.",
    "advanced.wildcardMode.sequentialTitle": "Sequential: use the same expansion algorithm and advance the seed by one after an accepted queue, wrapping to zero after the maximum.",
    "advanced.wildcardSeedTitle": "Deterministic selection seed. The same source and seed produce the same result; Sequential advances it by one after an accepted queue.",
    "advanced.pin": "Pin",
  ...
  ko: {
    ...
    "advanced.wildcardMode.populateTitle": "일반: 원본 필드를 결정적으로 확장합니다. 같은 원본과 같은 시드는 같은 결과를 만들고 시드는 유지되며, 워크플로우에는 원본 필드가 저장됩니다.",
    "advanced.wildcardMode.sequentialTitle": "순차: 같은 확장 알고리즘을 사용하고 수락된 큐 뒤 시드를 1 증가시키며, 최댓값 다음에는 0으로 순환합니다.",
    "advanced.wildcardSeedTitle": "결정적 선택 시드입니다. 같은 원본과 같은 시드는 같은 결과를 만들며, 순차는 수락된 큐 뒤 시드를 1 증가시킵니다.",
    "advanced.pin": "고정",
  ...
  ja: {
    ...
    "advanced.wildcardMode.populateTitle": "通常: 元フィールドを決定的に展開します。同じ元フィールドと同じシードは同じ結果になり、シードは維持され、ワークフローには元フィールドが保存されます。",
    "advanced.wildcardMode.sequentialTitle": "順次: 同じ展開アルゴリズムを使い、受理されたキュー後にシードを 1 増やし、最大値の次は 0 に循環します。",
    "advanced.wildcardSeedTitle": "決定的な選択シードです。同じ元フィールドと同じシードは同じ結果になり、順次は受理されたキュー後にシードを 1 増やします。",
    "advanced.pin": "固定",
  ...
  zh: {
    ...
    "advanced.wildcardMode.populateTitle": "普通：确定性展开源字段。相同源文本和相同种子会产生相同结果，种子保持不变，工作流保存源字段。",
    "advanced.wildcardMode.sequentialTitle": "顺序：使用相同展开算法，在队列接受后将种子加 1，超过最大值后循环到 0。",
    "advanced.wildcardSeedTitle": "确定性选择种子。相同源文本和相同种子会产生相同结果；顺序在队列接受后将种子加 1。",
    "advanced.pin": "固定",
