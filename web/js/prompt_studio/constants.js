...
const PROMPT_STUDIO_TEXT = {
  en: {
    ...
    "advanced.wildcardTitle": "Syntax: __name__, {a|b|c}, N::weight, {n$$...}/{min-max$$separator$$...}, and N#__name__. Nesting is supported; wildcard names ignore case and accept * globs; only line-start # comments are removed.",
    "advanced.wildcardSeed": "wildcard seed",
    "advanced.wildcardSeedControl": "seed control",
    "advanced.wildcardModeTitle": "Choose how wildcard source fields are expanded or reproduced.",
    "advanced.wildcardMode.populateTitle": "Populate: expand source fields with seed-based weighted random choices. The live editor keeps the source; saved workflow metadata stores expanded fields in Reproduce mode. Seed control selects the next live-run seed.",
    "advanced.wildcardMode.fixedTitle": "Fixed: EasyUse compatibility mode. It still expands source fields with seed-based weighted random choices; fixed only keeps the seed when seed control is fixed. Saved metadata stores expanded fields in Reproduce mode.",
    "advanced.wildcardMode.sequentialTitle": "Sequential: expand source fields from seed % option count (and seed % range width), then force the next live-run seed to increment. Saved metadata stores expanded fields in Reproduce mode.",
    "advanced.wildcardMode.reproduceTitle": "Reproduce: use the already saved/cached field text unchanged. No wildcard selection or seed advance occurs, and the mode remains Reproduce until you change it.",
    "advanced.wildcardSeedTitle": "Seed for weighted random choices. Sequential uses seed modulo each option count/range width; Reproduce performs no selection.",
    "advanced.wildcardSeedControlTitle": "Next live-run seed: fixed keeps it, randomize chooses a new value, and increment/decrement move by one with wraparound. Sequential forces increment; Reproduce does not advance it.",
    "advanced.pin": "Pin",
  ...
  ko: {
    ...
    "advanced.wildcardTitle": "문법: __name__, {a|b|c}, N::weight, {n$$...}/{min-max$$separator$$...}, N#__name__. 중첩을 지원하며 이름은 대소문자를 구분하지 않고 * glob을 사용할 수 있습니다. 줄 시작 # 주석만 제거됩니다.",
    "advanced.wildcardSeed": "와일드카드 시드",
    "advanced.wildcardSeedControl": "시드 제어",
    "advanced.wildcardModeTitle": "와일드카드 원본 필드를 확장하거나 저장 결과를 재현하는 방식을 고릅니다.",
    "advanced.wildcardMode.populateTitle": "일반 채우기: 원본 필드를 시드 기반 가중 랜덤으로 확장합니다. 현재 편집기는 원본을 유지하고, 저장 워크플로우 메타데이터에는 확장 필드가 재현 모드로 기록됩니다. 시드 제어가 다음 실시간 실행 시드를 정합니다.",
    "advanced.wildcardMode.fixedTitle": "고정: EasyUse 호환 모드입니다. 원본 필드를 계속 시드 기반 가중 랜덤으로 확장하며, 시드 제어도 fixed일 때만 시드가 유지됩니다. 저장 메타데이터에는 확장 필드가 재현 모드로 기록됩니다.",
    "advanced.wildcardMode.sequentialTitle": "순차: seed % 항목 수(범위는 seed % 범위 폭)로 원본 필드를 확장하고 다음 실시간 실행 시드를 강제로 1 증가시킵니다. 저장 메타데이터에는 확장 필드가 재현 모드로 기록됩니다.",
    "advanced.wildcardMode.reproduceTitle": "재현: 이미 저장되거나 캐시된 필드 텍스트를 그대로 사용합니다. 새 선택과 시드 증가는 없으며 사용자가 바꿀 때까지 재현 모드를 유지합니다.",
    "advanced.wildcardSeedTitle": "가중 랜덤 선택 시드입니다. 순차는 각 항목 수/범위 폭으로 나눈 나머지를 사용하고, 재현은 새 선택을 하지 않습니다.",
    "advanced.wildcardSeedControlTitle": "다음 실시간 실행 시드: fixed는 유지, randomize는 새 값, increment/decrement는 경계에서 순환하며 1씩 바꿉니다. 순차는 increment를 강제하고 재현은 시드를 진행하지 않습니다.",
    "advanced.pin": "고정",
  ...
  ja: {
    ...
    "advanced.wildcardTitle": "構文: __name__、{a|b|c}、N::weight、{n$$...}/{min-max$$separator$$...}、N#__name__。ネスト対応、名前は大文字小文字を区別せず * glob を使用可能。行頭の # コメントだけを削除します。",
    "advanced.wildcardSeed": "ワイルドカードシード",
    "advanced.wildcardSeedControl": "シード制御",
    "advanced.wildcardModeTitle": "ワイルドカードの元フィールドを展開するか、保存済み結果を再現する方法を選びます。",
    "advanced.wildcardMode.populateTitle": "通常展開: 元フィールドをシードによる重み付きランダム選択で展開します。ライブ編集画面は元データを保持し、保存ワークフローメタデータには展開済みフィールドを再現モードで記録します。シード制御が次のライブ実行シードを決めます。",
    "advanced.wildcardMode.fixedTitle": "固定: EasyUse 互換モードです。元フィールドは引き続きシードによる重み付きランダム選択で展開され、シード制御も fixed の場合だけシードを保持します。保存メタデータには展開済みフィールドを再現モードで記録します。",
    "advanced.wildcardMode.sequentialTitle": "順次: seed % 選択肢数（範囲は seed % 範囲幅）で元フィールドを展開し、次のライブ実行シードを必ず 1 増やします。保存メタデータには展開済みフィールドを再現モードで記録します。",
    "advanced.wildcardMode.reproduceTitle": "再現: 保存済みまたはキャッシュ済みのフィールド文字列をそのまま使用します。新しい選択やシード更新はなく、変更するまで再現モードを維持します。",
    "advanced.wildcardSeedTitle": "重み付きランダム選択のシードです。順次は各選択肢数/範囲幅の剰余を使い、再現は新しい選択を行いません。",
    "advanced.wildcardSeedControlTitle": "次のライブ実行シード: fixed は維持、randomize は新しい値、increment/decrement は境界で循環して 1 ずつ変更。順次は increment を強制し、再現は進めません。",
    "advanced.pin": "固定",
  ...
  zh: {
    ...
    "advanced.wildcardTitle": "语法：__name__、{a|b|c}、N::weight、{n$$...}/{min-max$$separator$$...}、N#__name__。支持嵌套；名称不区分大小写并可使用 * glob；只移除行首 # 注释。",
    "advanced.wildcardSeed": "通配符种子",
    "advanced.wildcardSeedControl": "种子控制",
    "advanced.wildcardModeTitle": "选择展开通配符源字段或复现已保存结果的方式。",
    "advanced.wildcardMode.populateTitle": "普通填充：按种子进行加权随机选择并展开源字段。实时编辑器保留源内容；保存的工作流元数据以复现模式记录展开后的字段。种子控制决定下一次实时运行的种子。",
    "advanced.wildcardMode.fixedTitle": "固定：EasyUse 兼容模式。仍会按种子进行加权随机选择并展开源字段；只有种子控制也是 fixed 时才保持种子。保存元数据以复现模式记录展开后的字段。",
    "advanced.wildcardMode.sequentialTitle": "顺序：使用 seed % 选项数（范围使用 seed % 范围宽度）展开源字段，并强制将下一次实时运行种子加 1。保存元数据以复现模式记录展开后的字段。",
    "advanced.wildcardMode.reproduceTitle": "复现：原样使用已保存或缓存的字段文本。不执行新的通配符选择，也不推进种子；在用户更改前会一直保持复现模式。",
    "advanced.wildcardSeedTitle": "加权随机选择使用的种子。顺序模式对每组选项数/范围宽度取余；复现模式不进行新选择。",
    "advanced.wildcardSeedControlTitle": "下一次实时运行种子：fixed 保持，randomize 选择新值，increment/decrement 在边界循环并加减 1。顺序强制 increment；复现不推进种子。",
    "advanced.pin": "固定",
