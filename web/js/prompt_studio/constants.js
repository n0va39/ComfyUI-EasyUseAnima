...
const PROMPT_STUDIO_TEXT = {
  en: {
    ...
    "advanced.wildcardHelpLabel": "Wildcard details",
    "advanced.wildcardHelp": "General\nThe same source, mode, and seed always produce the same result.\n\nSequential\nThe seed selects each option by position. A fixed seed reproduces the same result; choose Increment to move through the options.\n\nFixed\nKeep the current seed after an accepted queue in both General and Sequential modes.\n\nRandomize every time\nChoose a random seed for the next run after an accepted queue.\n\nIncrement\nAdd 1 to the seed after an accepted queue and wrap to 0 after the maximum.\n\nReuse previous seed\nRestore the seed and mode used by the most recently accepted queue and switch seed control to Fixed. Saved workflows store that reproducible seed in Fixed mode.\n\nSyntax\n__name__: load a file wildcard\n{a|b|c}: choose one dynamic option\nN::candidate: give a candidate weight N\n{n$$...}: choose n items\n{min-max$$separator$$...}: choose a ranged count with a separator\nN#__name__: choose N items from a file wildcard\nNested syntax is supported. Names ignore case and accept * globs. Only line-start # comments are removed.",
    "advanced.wildcardSeed": "wildcard seed",
    "advanced.wildcardModeTitle": "Choose General or Sequential wildcard expansion.",
    "advanced.wildcardMode.populateTitle": "General: expand the original source fields deterministically. The same source and seed produce the same result, and the workflow keeps the source fields.",
    "advanced.wildcardMode.sequentialTitle": "Sequential: use the seed as the option position. A fixed seed reproduces the same result; seed control decides what happens after a queue.",
    "advanced.wildcardSeedTitle": "Deterministic selection seed. The same source, mode, and seed produce the same result.",
    "advanced.wildcardSeedControl": "seed control",
    "advanced.wildcardSeedControlTitle": "Choose how the wildcard seed changes after an accepted queue.",
    ...
    "advanced.wildcardSeedControl.fixedTitle": "Keep the current seed. General and Sequential both reproduce the same result.",
    ...
    "advanced.wildcardPreviousSeed": "previous execution",
    "advanced.wildcardPreviousSeedShort": "Previous",
    "advanced.wildcardPreviousSeedReuse": "Reuse seed {seed}",
    "advanced.wildcardPreviousSeedTitle": "Restore previous execution seed {seed} and its mode, then switch seed control to Fixed.",
    "advanced.wildcardPreviousSeedUnavailable": "No previous seed",
    "advanced.wildcardPreviousSeedUnavailableTitle": "Run an accepted queue before reusing its seed.",
    "advanced.pin": "Pin",
    ...
    "advanced.swapOrientation": "Swap width and height",
    "advanced.swapOrientationTitle": "Swap the current width and height.",
    "advanced.swapOrientationSquareTitle": "Width and height are already equal.",
    "advanced.swapOrientationUnavailableTitle": "This bucket has no exact inverse preset. The current resolution is preserved.",
    "advanced.swapOrientationNaiaTitle": "NAIA provides the resolution at queue time, so orientation cannot be swapped here.",
    "advanced.swapOrientationLinkedTitle": "A linked input controls this resolution, so orientation cannot be swapped here.",
  ...
  ko: {
    ...
    "advanced.wildcardHelpLabel": "와일드카드 상세 규칙",
    "advanced.wildcardHelp": "일반\n같은 원본, 같은 모드, 같은 시드는 항상 같은 결과를 만듭니다.\n\n순차\n시드값을 항목의 순서 위치로 사용합니다. 시드를 고정하면 같은 결과를 재현하며, 항목을 순환하려면 증가를 선택합니다.\n\n고정\n큐가 정상 수락된 뒤에도 현재 시드를 유지합니다. 일반과 순차 모두 같은 결과를 다시 얻을 수 있습니다.\n\n매번 랜덤\n큐가 정상 수락되면 다음 실행에 사용할 시드를 무작위로 바꿉니다.\n\n증가\n큐가 정상 수락되면 시드를 1 올리고, 최댓값 다음에는 0으로 돌아갑니다.\n\n이전 시드 재사용\n가장 최근에 정상 수락된 큐의 시드와 모드를 복원하고 시드 제어를 고정으로 바꿉니다. 저장되는 워크플로우도 해당 재현 시드와 고정 모드로 저장됩니다.\n\n문법\n__name__: 파일 와일드카드 불러오기\n{a|b|c}: 동적 항목 하나 선택\nN::candidate: 후보 candidate의 가중치를 N으로 지정\n{n$$...}: n개 항목 선택\n{min-max$$separator$$...}: 범위 개수와 구분자를 지정해 선택\nN#__name__: 파일 와일드카드에서 N개 선택\n중첩을 지원하며 이름은 대소문자를 구분하지 않고 * glob을 사용할 수 있습니다. 줄 시작 # 주석만 제거됩니다.",
    "advanced.wildcardSeed": "와일드카드 시드",
    "advanced.wildcardModeTitle": "와일드카드 확장 방식을 일반 또는 순차에서 고릅니다.",
    "advanced.wildcardMode.populateTitle": "일반: 원본 필드를 결정적으로 확장합니다. 같은 원본과 같은 시드는 같은 결과를 만들며, 워크플로우에는 원본 필드가 저장됩니다.",
    "advanced.wildcardMode.sequentialTitle": "순차: 시드값을 항목의 순서 위치로 사용합니다. 고정 시드는 같은 결과를 재현하며, 큐 이후 동작은 시드 제어가 결정합니다.",
    "advanced.wildcardSeedTitle": "결정적 선택 시드입니다. 같은 원본, 같은 모드, 같은 시드는 같은 결과를 만듭니다.",
    "advanced.wildcardSeedControl": "시드 제어",
    "advanced.wildcardSeedControlTitle": "큐가 정상 수락된 뒤 와일드카드 시드를 어떻게 바꿀지 고릅니다.",
    ...
    "advanced.wildcardSeedControl.fixedTitle": "현재 시드를 유지합니다. 일반과 순차 모두 같은 결과를 다시 얻습니다.",
    ...
    "advanced.wildcardPreviousSeed": "이전 실행",
    "advanced.wildcardPreviousSeedShort": "이전",
    "advanced.wildcardPreviousSeedReuse": "시드 {seed} 재사용",
    "advanced.wildcardPreviousSeedTitle": "이전 실행의 시드 {seed}와 모드를 복원하고 시드 제어를 고정으로 바꿉니다.",
    "advanced.wildcardPreviousSeedUnavailable": "이전 시드 없음",
    "advanced.wildcardPreviousSeedUnavailableTitle": "정상 수락된 실행이 있어야 이전 시드를 재사용할 수 있습니다.",
    "advanced.pin": "고정",
    ...
    "advanced.swapOrientation": "너비와 높이 바꾸기",
    "advanced.swapOrientationTitle": "현재 너비와 높이를 서로 바꿉니다.",
    "advanced.swapOrientationSquareTitle": "너비와 높이가 이미 같습니다.",
    "advanced.swapOrientationUnavailableTitle": "이 버킷에는 정확히 반대인 프리셋이 없습니다. 현재 해상도를 유지합니다.",
    "advanced.swapOrientationNaiaTitle": "NAIA가 큐 실행 때 해상도를 제공하므로 여기서는 방향을 바꿀 수 없습니다.",
    "advanced.swapOrientationLinkedTitle": "연결된 입력이 이 해상도를 제어하므로 여기서는 방향을 바꿀 수 없습니다.",
  ...
  ja: {
    ...
    "advanced.wildcardHelpLabel": "ワイルドカード詳細",
    "advanced.wildcardHelp": "通常\n同じ元テキスト、モード、シードは常に同じ結果になります。\n\n順次\nシードを項目の順番として使います。シードを固定すると同じ結果を再現し、項目を巡回するには増加を選びます。\n\n固定\nキュー受理後も現在のシードを維持します。通常と順次の両方で同じ結果を再現できます。\n\n毎回ランダム\nキュー受理後、次回実行のシードをランダムに変更します。\n\n増加\nキュー受理後にシードを 1 増やし、最大値の次は 0 に戻ります。\n\n前回シードを再利用\n直近で受理されたキューのシードとモードを復元し、シード制御を固定に切り替えます。保存ワークフローも再現用シードと固定モードで保存されます。\n\n構文\n__name__: ファイルワイルドカードを読み込む\n{a|b|c}: 動的項目を 1 つ選ぶ\nN::candidate: 候補 candidate の重みを N に指定\n{n$$...}: n 個の項目を選ぶ\n{min-max$$separator$$...}: 範囲数と区切り文字を指定して選ぶ\nN#__name__: ファイルワイルドカードから N 個選ぶ\nネスト対応、名前は大文字小文字を区別せず * glob を使用可能。行頭の # コメントだけを削除します。",
    "advanced.wildcardSeed": "ワイルドカードシード",
    "advanced.wildcardModeTitle": "ワイルドカード展開を通常または順次から選びます。",
    "advanced.wildcardMode.populateTitle": "通常: 元フィールドを決定的に展開します。同じ元フィールドと同じシードは同じ結果になり、ワークフローには元フィールドが保存されます。",
    "advanced.wildcardMode.sequentialTitle": "順次: シードを項目の順番として使います。固定シードは同じ結果を再現し、キュー後の動作はシード制御が決めます。",
    "advanced.wildcardSeedTitle": "決定的な選択シードです。同じ元テキスト、モード、シードは同じ結果になります。",
    "advanced.wildcardSeedControl": "シード制御",
    "advanced.wildcardSeedControlTitle": "キュー受理後にワイルドカードシードをどう変更するか選びます。",
    ...
    "advanced.wildcardSeedControl.fixedTitle": "現在のシードを維持します。通常と順次の両方で同じ結果を再現します。",
    ...
    "advanced.wildcardPreviousSeed": "前回の実行",
    "advanced.wildcardPreviousSeedShort": "前回",
    "advanced.wildcardPreviousSeedReuse": "シード {seed} を再利用",
    "advanced.wildcardPreviousSeedTitle": "前回実行のシード {seed} とモードを復元し、シード制御を固定に切り替えます。",
    "advanced.wildcardPreviousSeedUnavailable": "前回シードなし",
    "advanced.wildcardPreviousSeedUnavailableTitle": "受理された実行後に前回シードを再利用できます。",
    "advanced.pin": "固定",
    ...
    "advanced.swapOrientation": "幅と高さを入れ替える",
    "advanced.swapOrientationTitle": "現在の幅と高さを入れ替えます。",
    "advanced.swapOrientationSquareTitle": "幅と高さはすでに同じです。",
    "advanced.swapOrientationUnavailableTitle": "このバケットには正確に反転したプリセットがありません。現在の解像度を維持します。",
    "advanced.swapOrientationNaiaTitle": "キュー実行時に NAIA が解像度を提供するため、ここでは向きを変更できません。",
    "advanced.swapOrientationLinkedTitle": "接続された入力がこの解像度を制御しているため、ここでは向きを変更できません。",
  ...
  zh: {
    ...
    "advanced.wildcardHelpLabel": "通配符详细规则",
    "advanced.wildcardHelp": "普通\n相同源文本、模式和种子始终产生相同结果。\n\n顺序\n使用种子作为选项位置。固定种子可复现相同结果；要循环选项请选择增加。\n\n固定\n队列接受后保持当前种子。普通和顺序模式都可复现相同结果。\n\n每次随机\n队列接受后为下次运行随机选择种子。\n\n增加\n队列接受后将种子加 1，超过最大值后回到 0。\n\n复用上次种子\n恢复最近一次已接受队列使用的种子和模式，并将种子控制切换为固定。保存的工作流也会使用该可复现种子和固定模式。\n\n语法\n__name__: 加载文件通配符\n{a|b|c}: 选择一个动态选项\nN::candidate: 将候选 candidate 的权重设为 N\n{n$$...}: 选择 n 个项目\n{min-max$$separator$$...}: 按范围数量和分隔符选择\nN#__name__: 从文件通配符选择 N 个项目\n支持嵌套；名称不区分大小写并可使用 * glob；只移除行首 # 注释。",
    "advanced.wildcardSeed": "通配符种子",
    "advanced.wildcardModeTitle": "从普通或顺序中选择通配符展开方式。",
    "advanced.wildcardMode.populateTitle": "普通：确定性展开源字段。相同源文本和相同种子会产生相同结果，工作流保存源字段。",
    "advanced.wildcardMode.sequentialTitle": "顺序：使用种子作为选项位置。固定种子可复现相同结果；队列后的行为由种子控制决定。",
    "advanced.wildcardSeedTitle": "确定性选择种子。相同源文本、模式和种子会产生相同结果。",
    "advanced.wildcardSeedControl": "种子控制",
    "advanced.wildcardSeedControlTitle": "选择队列接受后如何更改通配符种子。",
    "advanced.wildcardSeedControl.fixed": "固定",
    "advanced.wildcardSeedControl.randomize": "每次随机",
    "advanced.wildcardSeedControl.increment": "增加",
    "advanced.wildcardSeedControl.fixedTitle": "保持当前种子。普通和顺序模式都可复现相同结果。",
    ...
    "advanced.wildcardPreviousSeed": "上次执行",
    "advanced.wildcardPreviousSeedShort": "上次",
    "advanced.wildcardPreviousSeedReuse": "复用种子 {seed}",
    "advanced.wildcardPreviousSeedTitle": "恢复上次执行的种子 {seed} 和模式，并将种子控制切换为固定。",
    "advanced.wildcardPreviousSeedUnavailable": "没有上次种子",
    "advanced.wildcardPreviousSeedUnavailableTitle": "队列成功接受后才能复用上次种子。",
    "advanced.pin": "固定",
    ...
    "advanced.swapOrientation": "交换宽度和高度",
    "advanced.swapOrientationTitle": "交换当前的宽度和高度。",
    "advanced.swapOrientationSquareTitle": "宽度和高度已经相同。",
    "advanced.swapOrientationUnavailableTitle": "此分辨率桶中没有精确反转的预设。当前分辨率将保持不变。",
    "advanced.swapOrientationNaiaTitle": "NAIA 会在加入队列时提供分辨率，因此无法在此交换方向。",
    "advanced.swapOrientationLinkedTitle": "连接的输入正在控制此分辨率，因此无法在此交换方向。",
...
const ADVANCED_WILDCARD_DEFAULT_MODE = "일반";
